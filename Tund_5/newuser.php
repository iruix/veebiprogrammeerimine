...
require("functions.php");
  ...
  $passwordError = "";
	...
	//kui on uue kasutaja loomis nuppu vajutatud
  ...
  //kontrollime kas sünniaeg sisestati ja on korrektne
  ...
  //kontrollin kuupäeva õigsust
   ...
   //kui kõik on korras ss salvestame kasutaja
   if(empty($nameError) and empty($surnameError) and 
   empty($birthMonthError) and empty($birthYearError) 
   and empty($birthDayError) and empty($genderError) 
   and empty($emailError) and empty($passwordError)){
	$notice = signup($name, $surname, $email ,$gender, $birthDate, $_POST["password"]);
	echo $notice;
   }
  
  }//nupuvajutus saab läbi
...
<html>
  <head>
    <meta charset="utf-8">
	<title>Katselise veebi loomine</title>
  </head>
  <body>
    <h1>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <input name="surName" type="text" value=""><span><?php echo $surnameError;
	  ?></span><br>
