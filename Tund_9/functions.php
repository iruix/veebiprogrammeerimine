...
function readprofilecolors(){
  ...
  function storeuserprofile($desc, $bgcol, $txtcol){
	...
	if($stmt->fetch()){
		...
		$stmt->close();
		$stmt = $mysqli->prepare("UPDATE vpuserprofiles3 SET description=?, bgcolor=?, txtcolor=? WHERE id=?");
		echo $mysqli->error;
  ...
  function listusers(){
	...
	//$stmt = $mysqli->prepare("SELECT firstname, lastname, email, description FROM vpusers, vpuserprofiles WHERE vpuserprofiles3.userid=vpusers.id");
